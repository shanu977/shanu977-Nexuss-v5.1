...
const ADSTERRA_CONTAINER_ID = "container-9434692e0f33ebcfc9c000e143262e75";

describe("MessageList Adsterra status area", () => {
  it("shows the Adsterra banner when generation is idle", () => {
    const userMsg = makeMessage("u1", "c1", "user", "Explain hooks");
    const asstMsg = makeMessage("a1", "c1", "assistant", "Done.");
    ...
    expect(document.getElementById(ADSTERRA_CONTAINER_ID)).not.toBeNull();
    expect(screen.queryByText("Nexuss is thinking...")).not.toBeInTheDocument();
  });

  it("shows the Adsterra banner on the initial empty chat", () => {
    renderList({ messages: [] });

    expect(document.getElementById(ADSTERRA_CONTAINER_ID)).not.toBeNull();
  });

  it("hides the Adsterra banner and shows thinking once generation starts", () => {
    const userMsg = makeMessage("u1", "c1", "user", "Explain hooks");
    ...
    expect(screen.getByText("Nexuss is thinking...")).toBeInTheDocument();
    expect(document.getElementById(ADSTERRA_CONTAINER_ID)).toBeNull();
  });

  it("does not show the Adsterra banner while the response is streaming", () => {
    const userMsg = makeMessage("u1", "c1", "user", "Explain hooks");
    const asstMsg = makeMessage("a1", "c1", "assistant", "");
